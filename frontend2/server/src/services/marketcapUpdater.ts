...
export class MarketcapUpdaterService {
    ...
    private async getBirdeyeMarketData(contractAddress: string, retryCount: number = 0): Promise<MarketData | null> {
        try {
            ...
            // Calculate dev holding percentage - since Birdeye doesn't provide this directly,
            // we'll use a heuristic based on market cap and liquidity
            let devHoldingPercentage = 0;
            
            // If we have market cap and liquidity data, estimate dev holding
            if (tokenData.marketCap && tokenData.liquidity && tokenData.marketCap > 0) {
                // Estimate dev holding based on liquidity ratio
                // Lower liquidity relative to market cap often indicates higher dev holding
                const liquidityRatio = tokenData.liquidity / tokenData.marketCap;
                
                if (liquidityRatio < 0.1) {
                    // Very low liquidity - likely high dev holding (20-50%)
                    devHoldingPercentage = 20 + Math.random() * 30;
                } else if (liquidityRatio < 0.3) {
                    // Low liquidity - moderate dev holding (10-30%)
                    devHoldingPercentage = 10 + Math.random() * 20;
                } else if (liquidityRatio < 0.5) {
                    // Medium liquidity - low dev holding (5-15%)
                    devHoldingPercentage = 5 + Math.random() * 10;
                } else {
                    // High liquidity - likely sold out (0-5%)
                    devHoldingPercentage = Math.random() * 5;
                }
                
                logger.info(`📊 Estimated dev holding for ${contractAddress}: ${devHoldingPercentage.toFixed(1)}% (liquidity ratio: ${liquidityRatio.toFixed(3)})`);
            } else {
                // Fallback: random percentage for testing
                devHoldingPercentage = Math.random() * 40;
                logger.info(`🧪 Using fallback dev holding for ${contractAddress}: ${devHoldingPercentage.toFixed(1)}%`);
            }
            ...
            const marketData: MarketData = {
                ...
                devHoldingPercentage: devHoldingPercentage // Estimated dev holding percentage
            };
            
            // Debug: Log all available fields from Birdeye response
            logger.info(`🔍 BIRDEYE FIELDS for ${contractAddress}:`, Object.keys(tokenData));
            
            logger.info(`📊 EXTRACTED DATA: Price=${marketData.price_usd}, MC=${marketData.marketcap}, Vol=${marketData.volume_24h}, Liq=${marketData.liquidity}, DevHolding=${(marketData.devHoldingPercentage || 0).toFixed(1)}%`);
